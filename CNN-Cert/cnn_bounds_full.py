...
from cnn_bounds_full_core import (
    pool,
    conv,
    conv_bound,
    conv_full,
        ...
        while i < len(model.layers):
            ...
            if type(layer) == Conv2D:
                ...
                if padding == "same":
                    ...
                    total_padding_h = (
                        stride[0] * (desired_h - 1) + W.shape[0] - cur_shape[0]
                    )
                    total_padding_w = (
                        stride[1] * (desired_w - 1) + W.shape[1] - cur_shape[1]
                    )
                    pad = (
                        int(np.floor(total_padding_h / 2)),
                        int(np.ceil(total_padding_h / 2)),
                        int(np.floor(total_padding_w / 2)),
                        int(np.ceil(total_padding_w / 2)),
                    )
                cur_shape = (
                    int((cur_shape[0] + pad[0] + pad[1] - W.shape[0]) / stride[0]) + 1,
                    int((cur_shape[1] + pad[2] + pad[3] - W.shape[1]) / stride[1]) + 1,
                    W.shape[-1],
                )
                ...
                stride = layer.get_config()["strides"]
                W = np.zeros(
                    (pool_size[0], pool_size[1], cur_shape[2], cur_shape[2]),
                    dtype=np.float32,
                )
                for f in range(W.shape[2]):
                ...
                if padding == "same":
                    ...
                    desired_w = int(np.ceil(cur_shape[0] / stride[1]))
                    total_padding_h = (
                        stride[0] * (desired_h - 1) + pool_size[0] - cur_shape[0]
                    )
                    total_padding_w = (
                        stride[1] * (desired_w - 1) + pool_size[1] - cur_shape[1]
                    )
                    pad = (
                        int(np.floor(total_padding_h / 2)),
                        int(np.ceil(total_padding_h / 2)),
                ...
                cur_shape = (
                    int((cur_shape[0] + pad[0] + pad[1] - pool_size[0]) / stride[0])
                    + 1,
                    int((cur_shape[1] + pad[2] + pad[3] - pool_size[1]) / stride[1])
                    + 1,
                    cur_shape[2],
                )
                W = np.ascontiguousarray(W.transpose((3, 0, 1, 2)).astype(np.float32))
                ...
                if padding1 == "same":
                    ...
                    total_padding_h = (
                        stride1[0] * (desired_h - 1) + W1.shape[0] - cur_shape[0]
                    )
                    total_padding_w = (
                        stride1[1] * (desired_w - 1) + W1.shape[1] - cur_shape[1]
                    )
                    pad1 = (
                        int(np.floor(total_padding_h / 2)),
                        int(np.ceil(total_padding_h / 2)),
                ...
                cur_shape = (
                    int((cur_shape[0] + pad1[0] + pad1[1] - W1.shape[0]) / stride1[0])
                    + 1,
                    int((cur_shape[1] + pad1[2] + pad1[3] - W1.shape[1]) / stride1[1])
                    + 1,
                    W1.shape[3],
                )
                ...
                if padding2 == "same":
                    ...
                    total_padding_h = (
                        stride2[0] * (desired_h - 1) + W2.shape[0] - cur_shape[0]
                    )
                    total_padding_w = (
                        stride2[1] * (desired_w - 1) + W2.shape[1] - cur_shape[1]
                    )
                    pad2 = (
                        int(np.floor(total_padding_h / 2)),
                        int(np.ceil(total_padding_h / 2)),
                ...
                if padding3 == "same":
                    ...
                    total_padding_h = (
                        stride3[0] * (desired_h - 1) + W3.shape[0] - cur_shape[0]
                    )
                    total_padding_w = (
                        stride3[1] * (desired_w - 1) + W3.shape[1] - cur_shape[1]
                    )
                    pad3 = (
                        int(np.floor(total_padding_h / 2)),
                        int(np.ceil(total_padding_h / 2)),
...
def UL_conv_bound(A, B, pad, stride, shape, W, b, inner_pad, inner_stride, inner_shape):
    A_new = np.zeros(
        (
            ...
            A.shape[1],
            A.shape[2],
            inner_stride[0] * (A.shape[3] - 1) + W.shape[1],
            inner_stride[1] * (A.shape[4] - 1) + W.shape[2],
            W.shape[3],
        ),
        dtype=np.float32,
    )
    ...
    for x in range(A_new.shape[0]):
        ...
        t_start = np.maximum(
            0,
            -stride[0] * inner_stride[0] * x + inner_stride[0] * pad[0] + inner_pad[0],
        )
        t_end = np.minimum(
            A_new.shape[3],
            inner_shape[0]
            - stride[0] * inner_stride[0] * x
            + inner_stride[0] * pad[0]
            + inner_pad[0],
        )
        for y in range(A_new.shape[1]):
            ...
            u_start = np.maximum(
                0,
                -stride[1] * inner_stride[1] * y
                + inner_stride[1] * pad[2]
                + inner_pad[2],
            )
            u_end = np.minimum(
                A_new.shape[4],
                ...
                for u in range(u_start, u_end):
                    for p in range(p_start, p_end):
                        for q in range(q_start, q_end):
                            if (
                                0 <= t - inner_stride[0] * p < W.shape[1]
                                and 0 <= u - inner_stride[1] * q < W.shape[2]
                            ):
                                A_new[x, y, :, t, u, :] += np.dot(
                                    A[x, y, :, p, q, :],
                                    W[
                                        :,
                                        t - inner_stride[0] * p,
...
def UL_basic_block_2_bound(
    ...
    inter_pad = (
        stride2[0] * pad[0] + pad2[0],
        stride2[0] * pad[1] + pad2[1],
        stride2[1] * pad[2] + pad2[2],
        stride2[1] * pad[3] + pad2[3],
    )
    ...
    if upper:
        A1, B1 = UL_relu_bound(
            A1,
            B1,
            np.asarray(inter_pad),
            np.asarray(inter_stride),
            alpha_u,
            alpha_l,
            beta_u,
            beta_l,
...
def UL_basic_block_bound(
    A, B, pad, stride, W1, W2, b1, b2, pad1, pad2, stride1, stride2, upper=True
):
...
def UL_pool_bound(
    ...
    A_new = np.zeros(
        (
            ...
            A.shape[1],
            A.shape[2],
            inner_stride[0] * (A.shape[3] - 1) + pool_size[0],
            inner_stride[1] * (A.shape[4] - 1) + pool_size[1],
            A.shape[5],
        ),
        dtype=np.float32,
    )
        ...
        for y in range(A_new.shape[1]):
            for t in range(A_new.shape[3]):
                for u in range(A_new.shape[4]):
                    ...
                    if (
                        0 <= inner_index_x < inner_shape[0]
                        and 0 <= inner_index_y < inner_shape[1]
                    ):
                        for p in range(A.shape[3]):
                            for q in range(A.shape[4]):
                                if (
                                    0 <= t - inner_stride[0] * p < alpha_u.shape[0]
                                    and 0 <= u - inner_stride[1] * q < alpha_u.shape[1]
                                    and 0
                                    <= p + stride[0] * x - pad[0]
                                    < alpha_u.shape[2]
                                    and 0
    ...
    B_new += conv_full(A_plus, beta_u, pad, stride) + conv_full(
        A_minus, beta_l, pad, stride
    )
    return A_new, B_new
...
def compute_bounds(
    ...
    if types[nlayer - 1] == "relu":
        ...
        # print('conv')
        A_u = weights[nlayer - 1].reshape(
            (
                1,
                1,
                weights[nlayer - 1].shape[0],
                weights[nlayer - 1].shape[1],
                weights[nlayer - 1].shape[2],
                weights[nlayer - 1].shape[3],
            )
        ) * np.ones(
            (
                out_shape[0],
        ...
        B_u = biases[nlayer - 1] * np.ones(
            (out_shape[0], out_shape[1], out_shape[2]), dtype=np.float32
        )
        A_l = A_u.copy()
        B_l = B_u.copy()
        pad = pads[nlayer - 1]
        ...
        A_u = np.eye(out_shape[2]).astype(np.float32).reshape(
            (1, 1, out_shape[2], 1, 1, out_shape[2])
        ) * np.ones(
            (out_shape[0], out_shape[1], out_shape[2], 1, 1, out_shape[2]),
            dtype=np.float32,
        )
        ...
        pad = (0, 0, 0, 0)
        stride = (1, 1)
        alpha_u, alpha_l, beta_u, beta_l = pool_linear_bounds(
            LBs[nlayer - 1],
            UBs[nlayer - 1],
            np.asarray(pads[nlayer - 1]),
            np.asarray(strides[nlayer - 1]),
            np.asarray(sizes[nlayer - 1]),
        )
        A_u, B_u = UL_pool_bound(
            A_u,
            B_u,
            np.asarray(pad),
            np.asarray(stride),
            ...
            np.asarray(pads[nlayer - 1]),
            np.asarray(strides[nlayer - 1]),
            np.asarray(LBs[nlayer - 1].shape),
            alpha_u,
            alpha_l,
            beta_u,
            beta_l,
        )
        A_l, B_l = UL_pool_bound(
            A_l,
            B_l,
            np.asarray(pad),
            np.asarray(stride),
        ...
        pad = (0, 0, 0, 0)
        stride = (1, 1)
        A_u, B_u = UL_basic_block_2_bound(
            A_u,
            B_u,
            pad,
            stride,
        ...
        A_l, B_l = UL_basic_block_2_bound(
            A_l,
            B_l,
            pad,
            stride,
        ...
        pad = (
            inner_stride[0] * pad[0] + inner_pad[0],
            inner_stride[0] * pad[1] + inner_pad[1],
            inner_stride[1] * pad[2] + inner_pad[2],
            inner_stride[1] * pad[3] + inner_pad[3],
        )
    ...
    for i in range(nlayer - 2, -1, -1):
        if types[i] == "conv":
            # print('conv')
            A_u, B_u = UL_conv_bound(
                A_u,
                B_u,
                np.asarray(pad),
                np.asarray(stride),
                np.asarray(UBs[i + 1].shape),
                weights[i],
                biases[i],
                np.asarray(pads[i]),
                np.asarray(strides[i]),
                np.asarray(UBs[i].shape),
            )
            A_l, B_l = UL_conv_bound(
                A_l,
                B_l,
                np.asarray(pad),
                np.asarray(stride),
                np.asarray(LBs[i + 1].shape),
                weights[i],
                biases[i],
                np.asarray(pads[i]),
                np.asarray(strides[i]),
                np.asarray(LBs[i].shape),
            )
            pad = (
                strides[i][0] * pad[0] + pads[i][0],
                strides[i][0] * pad[1] + pads[i][1],
            ...
            stride = (inner_stride[0] * stride[0], inner_stride[1] * stride[1])
    LUB, UUB = conv_bound_full(
        A_u, B_u, np.asarray(pad), np.asarray(stride), x0, eps, p_n
    )
    LLB, ULB = conv_bound_full(
        A_l, B_l, np.asarray(pad), np.asarray(stride), x0, eps, p_n
    )
    return LLB, UUB


# Main function to find output bounds
def find_output_bounds(
    weights, biases, shapes, pads, strides, sizes, types, x0, eps, p_n
):
    LBs = [x0 - eps]
    UBs = [x0 + eps]
...
def warmup(model, x, eps_0, p_n, func):
    ...
    last_weight = np.ascontiguousarray(
        (W[0, :, :, :]).reshape([1] + list(W.shape[1:])), dtype=np.float32
    )
...
def run(
    file_name, n_samples, p_n, q_n, activation="relu", cifar=False, tinyimagenet=False
):
    np.random.seed(1215)
    ...
    for i in range(len(inputs)):
        ...
        for j in range(steps):
            ...
            print(
                "Step {}, eps = {:.5f}, {:.6s} <= f_c - f_t <= {:.6s}".format(
                    j, np.exp(log_eps), str(np.squeeze(LB)), str(np.squeeze(UB))
                )
            )
        ...
        print(
            "[L1] method = CNN-Cert-{}, model = {}, image no = {}, true_id = {}, target_label = {}, true_label = {}, norm = {}, robustness = {:.5f}".format(
                activation,
                file_name,
                i,
                true_ids[i],
    ...
    print(
        "[L0] method = CNN-Cert-{}, model = {}, total images = {}, norm = {}, avg robustness = {:.5f}, avg runtime = {:.2f}".format(
            activation, file_name, len(inputs), str_p_n, eps_avg, total_time
        )
    )
...
# Main function to compute CNN-Cert bound
def run_gtsrb(model, input, label, eps, p_n, activation="relu"):
    np.random.seed(1215)
    tf.random.set_seed(1215)
    random.seed(1215)
    ...
    weights.append(last_weight)
    biases.append(np.asarray([b[label]]))
    shapes.append((1, 1, 1))

    LBs, UBs = find_output_bounds(
        weights,
        biases,
        shapes,
        model.pads,
