    ...
    def __init__(self, model, inp_shape=(28, 28, 1)):
        ...
        for layer in model.layers:
            ...
            if type(layer) == Conv2D:
                ...
                self.shapes.append(cur_shape)
                self.weights.append(
                    np.full(pool_size + (1, 1), np.nan, dtype=np.float32)
                )
        ...
        for i in range(len(self.weights)):
            self.weights[i] = np.ascontiguousarray(
                self.weights[i].transpose((3, 0, 1, 2)).astype(np.float32)
            )
...
def conv(W, x, pad, stride):
    ...
    y = np.zeros(
        (
            int((x.shape[0] - W.shape[1] + p_hl + p_hr) / s_h) + 1,
            int((x.shape[1] - W.shape[2] + p_wl + p_wr) / s_w) + 1,
            W.shape[0],
        ),
        dtype=np.float32,
    )
                ...
                for i in range(W.shape[1]):
                    for j in range(W.shape[2]):
                        for k in range(W.shape[3]):
                            if (
                                0 <= s_h * a + i - p_hl < x.shape[0]
                                and 0 <= s_w * b + j - p_wl < x.shape[1]
                            ):
                                y[a, b, c] += (
                                    W[c, i, j, k]
                                    * x[s_h * a + i - p_hl, s_w * b + j - p_wl, k]
                                )
    return y
...
def pool(pool_size, x0, pad, stride):
    ...
    y0 = np.zeros(
        (
            int((x0.shape[0] + p_hl + p_hr - pool_size[0]) / s_h) + 1,
            int((x0.shape[1] + p_wl + p_wr - pool_size[1]) / s_w) + 1,
            x0.shape[2],
        ),
        dtype=np.float32,
    )
    for x in range(y0.shape[0]):
        for y in range(y0.shape[1]):
            for r in range(y0.shape[2]):
                cropped = LB[
                    s_h * x - p_hl : pool_size[0] + s_h * x - p_hl,
                    s_w * y - p_wl : pool_size[1] + s_w * y - p_wl,
                    r,
                ]
...
@njit
def upper_bound_conv(
    A, B, pad, stride, W, b, inner_pad, inner_stride, inner_shape, LB, UB
):
    A_new = np.zeros(
        (
            A.shape[0],
    ...
    B_new = (
        conv_full(A_plus, alpha_u * b + beta_u, pad, stride)
        + conv_full(A_minus, alpha_l * b + beta_l, pad, stride)
        + B
    )
    return A_new, B_new
...
def lower_bound_conv(
    ...
    B_new = (
        conv_full(A_plus, alpha_l * b + beta_l, pad, stride)
        + conv_full(A_minus, alpha_u * b + beta_u, pad, stride)
        + B
    )
    return A_new, B_new
...
def pool_linear_bounds(LB, UB, pad, stride, pool_size):
    ...
    alpha_u = np.zeros(
        (
            pool_size[0],
            pool_size[1],
            int((UB.shape[0] + p_hl + p_hr - pool_size[0]) / s_h) + 1,
            int((UB.shape[1] + p_wl + p_wr - pool_size[1]) / s_w) + 1,
            UB.shape[2],
        ),
        dtype=np.float32,
    )
    ...
    alpha_l = np.zeros(
        (
            pool_size[0],
            pool_size[1],
            int((LB.shape[0] + p_hl + p_hr - pool_size[0]) / s_h) + 1,
            int((LB.shape[1] + p_wl + p_wr - pool_size[1]) / s_w) + 1,
            LB.shape[2],
        ),
        dtype=np.float32,
    )
...
@njit
def upper_bound_pool(
    A, B, pad, stride, pool_size, inner_pad, inner_stride, inner_shape, LB, UB
):
    A_new = np.zeros(
        (
            A.shape[0],
    ...
    alpha_u, alpha_l, beta_u, beta_l = pool_linear_bounds(
        LB, UB, inner_pad, inner_stride, pool_size
    )
    ...
    B_new = (
        conv_full(A_plus, beta_u, pad, stride)
        + conv_full(A_minus, beta_l, pad, stride)
        + B
    )
    return A_new, B_new
...
def lower_bound_pool(
    ...
    B_new = (
        conv_full(A_plus, beta_l, pad, stride)
        + conv_full(A_minus, beta_u, pad, stride)
        + B
    )
    return A_new, B_new
...
@njit
def compute_bounds(
    weights, biases, out_shape, nlayer, x0, eps, p_n, strides, pads, LBs, UBs
):
    pad = (0, 0, 0, 0)
    stride = (1, 1)
        ...
        if not np.isnan(weights[i]).any():  # Conv
            if i == nlayer - 1:
                ...
                A_u, B_u = upper_bound_conv(
                    ...
                    pads[i],
                    strides[i],
                    modified_UBs[i].shape,
                    modified_LBs[i + 1],
                    modified_UBs[i + 1],
                )
                A_l, B_l = lower_bound_conv(
                    A_l,
                    B_l,
                    pad,
                    stride,
            ...
            A_u, B_u = upper_bound_pool(
                ...
                strides[i],
                modified_UBs[i].shape,
                np.maximum(modified_LBs[i], 0),
                np.maximum(modified_UBs[i], 0),
            )
            A_l, B_l = lower_bound_pool(
                A_l,
                B_l,
                pad,
                stride,
